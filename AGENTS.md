...
12. **Two connection paths with two id semantics in App.tsx (root cause of several connection bugs)**: The app starts connections from two places with different tab-id schemes: (a) the **dialog path** — `ConnectionDialog` itself runs `invoke('ssh_connect')` (auth only) then calls `onConnect` with `config.id` (the saved connection's id); (b) the **App-orchestrated path** (`handleConnectionConnect` / restore / quick-connect) — App runs `ssh_connect` itself and uses a fresh `sessionId = ${connection.id}-dup-${Date.now()}` to avoid reusing a stale backend session. Consequences: tab id ≠ connection id for path (b) (breaks the sidebar green-dot check `activeConnections.has(connection.id)`); `handleConnectionDialogConnect`'s existing-tab branch (`tab.id === config.id`) is only reachable via the dialog path, so SSH never reaches it today (its missing SSH branch is a defensive gap, not a live bug — see `.claude/plans/fix-ssh-reconnect-stuck-connecting.md`). **When touching connection logic, always ask "which path and which id semantics?"**; a unique sessionId is also why a fresh tab should be created instead of mutating an existing one. Related backend facts: SSH auth happens in the dialog, so the App-side reconnect only needs `RECONNECT_TAB` (remount) — never a second `ssh_connect`; `handleReconnect` differs because it must first `ssh_disconnect` a possibly-dead session.
