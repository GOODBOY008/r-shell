...
pnpm run version:verify "v2.8.0-beta.1"   # check a tag matches every version file
...
13. **Failed restore leaves the tab mounted and firing StartPty with no backend session**: A restored tab whose connect fails stays `pending` and is added to `failedPendingTabIds` (App.tsx `connectRestoredTab`), so its placeholder shows a Connect button — `PtyTerminal` never mounts and the misleading "Connection not found" PTY error is avoided. When debugging "restore failed but reconnect works", look past any PTY error to the underlying `ssh_connect` error (e.g. the backend's aggressive 3 s connect timeout in `ssh/mod.rs`, which flakes on slow hosts). Startup restore is Chrome-style lazy (App.tsx): only each group's active tab reconnects eagerly; other restored tabs stay `pending` in `lazyRestoreTabIdsRef` and reconnect on first activation via the lazy-restore effect.
14. **Dev/e2e builds never touch the OS keychain nor auto-check for updates**: `master_key()` in `src-tauri/src/commands.rs` bypasses keyring whenever `tauri::is_dev()` or `RSHELL_DISABLE_KEYCHAIN=1`, keeping the master key in `dev-master-key.dat` under the app-data dir instead; likewise `get_update_context` reports `autoCheckDisabled: true` (via `tauri::is_dev()` or `RSHELL_DISABLE_AUTO_UPDATE=1`) so `UpdateChecker` skips its startup auto-check while manual checks still work. Both exist so unattended AI-agent/e2e runs don't stall on macOS keychain authorization prompts (each dev rebuild changes the ad-hoc signature, invalidating the previous "Always Allow") or trip over update dialogs/manifest fetches mid-run. Secrets sealed by a pre-bypass dev build fail to open once — re-enter the credential and it re-seals under the file key.
